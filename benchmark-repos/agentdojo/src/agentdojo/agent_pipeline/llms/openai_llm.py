...
def _message_to_openai(message: ChatMessage, model_name: str) -> ChatCompletionMessageParam:
    match message["role"]:
        case "system":
            # Native OpenAI reasoning/chat models support the newer
            # `developer` role, but vLLM and most OpenAI-compatible local
            # endpoints only accept `system`. AgentDojo's upstream default
            # always emits `developer`, which causes local Qwen/vLLM agents
            # to fail with "Unexpected message role".
...
def _assistant_message_to_content(
    ...
    # Some OpenAI-compatible Qwen3 endpoints place the assistant text in
    # reasoning_content / reasoning while leaving content=None. Only use that
    # fallback when the turn is not a tool-call turn.
    if tool_calls:
        return None
...
def _parse_qwen3_reasoning_tool_calls(message: ChatCompletionMessage) -> list[FunctionCall]:
    raw = (
        getattr(message, "reasoning_content", None)
        or getattr(message, "reasoning", None)
        or ""
    ...
    for idx, payload in enumerate(re.findall(r"<tool_call>\s*(\{.*?\})\s*</tool_call>", raw, flags=re.DOTALL)):
        try:
...
def _openai_to_assistant_message(message: ChatCompletionMessage) -> ChatAssistantMessage:
    if message.tool_calls is not None:
        ...
        tool_calls = _parse_qwen3_reasoning_tool_calls(message) or None
    return ChatAssistantMessage(
        role="assistant",
        content=_assistant_message_to_content(message, tool_calls),
        tool_calls=tool_calls,
    )

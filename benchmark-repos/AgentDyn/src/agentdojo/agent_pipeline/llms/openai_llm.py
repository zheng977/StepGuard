...
def _message_to_openai(message: ChatMessage, model_name: str) -> ChatCompletionMessageParam:
    match message["role"]:
        case "system":
            # Only use 'developer' role for native OpenAI models that support it;
            # vLLM and other OpenAI-compatible endpoints only accept 'system'.
...
class OpenAILLM(BasePipelineElement):
    ...
    def query(
        ...
        persisted_messages = [
            ...
            if not message.get("agentguard_ephemeral_feedback")
        ]
        messages = [*persisted_messages, output]
